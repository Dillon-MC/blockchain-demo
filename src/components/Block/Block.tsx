...
const Block = ({blockData, selectedPeer}: {blockData: IBlockProps, selectedPeer: number}): JSX.Element => {
    ...
    const updateBlock = (data: string): void => {
        ...
        for(let i = blockData.index; i < appContext.chains[selectedPeer].length; i++) {
    ...
    const mineBlock = () => {
        appContext.chains[selectedPeer][blockData.index].hash = createHash(blockData);
    }
    ...
    return (
        <div className="block">
            ...
            <input className="blockdatainput" type="text" value={blockData.data} onChange={(e) => updateBlock(e.target.value)}/>
